# Trading Records Management System

This document provides the SQL schema and implementation details for the Trading Records Management system in the Alphaxine ERP Finance Module.

## Database Schema

The following SQL script creates the necessary tables to track trading activities, profits/losses, and calculate key metrics like average profit, maximum profit, winning percentage, and drawdown.
...
-- Sample insert to demonstrate how to record a trade
-- INSERT INTO trade_records (trade_date, ticker, entry_price, exit_price, quantity, trade_type, profit_loss, trade_status, notes)
-- VALUES ('2025-07-07', 'TCS', 3500.00, 3550.00, 10, 'BUY', 500.00, 'PROFIT', 'Earnings momentum trade');

-- To update the metrics manually (if needed)
-- CALL update_trading_summary();
```

## SQL Queries for Retrieving Metrics

Here are some useful SQL queries to retrieve the trading metrics:
...
SELECT 
    ...
    net_pnl
FROM trading_metrics_daily
WHERE trade_date BETWEEN '2025-06-01' AND '2025-07-07'
ORDER BY trade_date;
```
...
To implement this in your Flask application, you'll need to:

1. Create database connection utilities
2. Create API endpoints to submit trades and retrieve metrics
3. Update the `records.html` template to display metrics and add forms for trade entry

## Frontend Requirements

For the frontend:

1. Use Chart.js for visualization (https://www.chartjs.org/)
2. Add a trade entry form to submit new trades
3. Display metrics in a dashboard format
...
1. Run the SQL script in your MySQL Workbench to create the database structure
2. Update the Flask application with the new endpoints
3. Install Chart.js in the frontend
4. Deploy the updated pages
...
- **Winning Percentage**: (Number of profitable trades / Total trades) × 100
- **Drawdown**: Maximum consecutive losing days
- **Net P&L**: Total profit - Total loss

These metrics will update automatically when new trades are added to the system.
